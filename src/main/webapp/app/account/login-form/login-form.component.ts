    ...
    const doLogin = async () => {
      ...
      try {
        ...
        if (bearerToken && bearerToken.slice(0, 7) === 'Bearer ') {
          ...
          if (rememberMe.value) {
            localStorage.setItem('primo-authenticationToken', jwt);
            sessionStorage.removeItem('primo-authenticationToken');
          } else {
            sessionStorage.setItem('primo-authenticationToken', jwt);
            localStorage.removeItem('primo-authenticationToken');
          }
        }
