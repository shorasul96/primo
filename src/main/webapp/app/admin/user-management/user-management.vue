  ...
  <div>
    <h2>
      ...
      <div class="d-flex justify-content-end">
        ...
        <router-link custom v-slot="{ navigate }" :to="{ name: 'PrimoUserCreate' }">
    ...
    <div class="table-responsive" v-if="users">
      <table class="table table-striped" aria-describedby="Users">
        ...
        <tbody v-if="users">
          <tr v-for="user in users" :key="user.id" :id="user.login">
            <td>
              <router-link :to="{ name: 'PrimoUserView', params: { userId: user.login } }">{{ user.id }}</router-link>
            </td>
            <td>{{ user.login }}</td>
      ...
      <b-modal ref="removeUser" id="removeUser" v-bind:title="t$('entity.delete.title')" @ok="deleteUser()">
        <div class="modal-body">
          <p id="primo-delete-user-heading" v-text="t$('userManagement.delete.question', { login: removeId })"></p>
        </div>
